...
function requireInvalidationHint(value: unknown): ControlPlaneInvalidationHint {
  ...
  if (!['connected', 'authority_changed', 'tab_changed', 'run_changed', 'template_changed', 'preset_changed', 'runtime_recovered'].includes(String(candidate.reason))) throw new ContractError(ERROR_CODES.invalidMessage, 'invalid invalidation reason');
