...
export class RunRuntimeServer {
  ...
  async handle(raw:unknown):Promise<unknown>{let request:RequestEnvelope|undefined;try{request=requireRequest(raw);if(request.source!=='sidepanel')throw new ContractError(ERROR_CODES.invalidMessage,'run commands must originate from sidepanel');const manager=await this.#manager();const payload=request.payload as Record<string,unknown>;switch(request.operation){
    ...
    case RUN_RUNTIME_OPERATIONS.rebind:{if(request.intent!=='command')throw new ContractError(ERROR_CODES.invalidMessage,'run.rebind must be a command');const runId=requireRunId(payload);const result=await manager.rebind(runId,payload.expectedGeneration,request.requestId,payload.targetTabId,payload.targetWindowId);return createSuccessResponse(request,{run:result.snapshot,idempotent:result.idempotent});}
